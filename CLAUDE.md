...
- 강의 보고 코드를 개선했을 때는 해당 문제 md 파일에 "코드 리팩토링" 항목 추가

## 기록 규칙 (역할 분리)

**풀이 디테일 = 주차 폴더의 문제별 md 파일**
- 위치: `N주차_<주제>/BOJ_<번호>_<이름>.md`
- 내용: 날짜, 유형, 핵심 테크닉, 잘한 부분, 막혔던 포인트, 배운 점, (있다면) 코드 리팩토링
- 매일 문제를 풀 때마다 해당 파일을 새로 만들거나 갱신
- 같은 문제 재풀이 시: 기존 파일에 "## 재풀이 (YYYY-MM-DD)" 섹션 추가

**진도/스케줄 = PROGRESS.md**
- 현재 상태 (날짜, 진도, 다음 할 일)
- 복귀 일정 표
- 스케줄 슬립 로그
- 일일 기록: "날짜 — 푼 문제 목록 + 주차 폴더 파일 링크" 한 줄짜리
- **풀이 디테일/막힌 점/배운 점 등은 PROGRESS.md에 넣지 않는다** (주차 폴더로)
